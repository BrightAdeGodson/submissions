...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem borderItem = new JMenuItem("Toggle border...");
			borderItem.addActionListener(menuHandler);
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			File outFile = fileChooser.getOutputFile(this, "Select Text File Name", "text.txt");
			if (outFile == null) {
				return;
			}
			try {
				BufferedWriter buw = new BufferedWriter(new FileWriter(outFile));
				buw.write(canvas.getHeight() + "~" + canvas.getWidth() + "~" + canvas.getBackground().getRGB());

				for (DrawTextItem draw : theString) {
					buw.write(System.lineSeparator());
					buw.write(draw.getString() + "~" + draw.getX() + "~" + draw.getY() + "~" + draw.getTextColor().getRGB() + "~" + draw.getRotationAngle());
				}
				buw.close();
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "Sorry,  an error occured while trying to save the image:\n" + e);
			}
		}
		else if (command.equals("Open...")) { // read a previously saved file, and reconstruct the list of strings
			File outfile = fileChooser.getInputFile(this, "Select Text File Name");
			if (outfile == null) {
				return;
			}
			try {
				...
				canvas.setSize(Integer.parseInt(str.split("~")[1]),
						Integer.parseInt(str.split("~")[0]));
				canvas.setBackground(new Color(Integer.parseInt(str.split("~")[2])));
				theString = new ArrayList<DrawTextItem>();
				while (sc.hasNextLine()) {
					String line = sc.nextLine();
					DrawTextItem draw = new DrawTextItem(line.split("~")[0]);
					draw.setX(Integer.parseInt(line.split("~")[1]));
					draw.setY(Integer.parseInt(line.split("~")[2]));
					draw.setTextColor(newColor(Integer.parseInt(line.split("~")[3])));
					draw.setRotationAngle(Double.parseDouble(line.split("~")[4]));
					theString.add(draw);
				}
				sc.close();
			} catch (Exception e) {
		...
		else if (command.equals("Toggle border...")) {
			if (border) {
				border = false;
			} else {
				border = true;
	...
	private Color newColor(int parseInt) {
